...
export function Kbd({children}: {children: ReactNode}) {
  return (
    <kbd className="tw-inline-flex tw-h-6 tw-min-w-[24px] tw-items-center tw-justify-center tw-rounded-md tw-border tw-border-b-2 tw-border-slate-300 tw-bg-slate-100 tw-px-1.5 tw-font-mono tw-text-xs tw-font-semibold tw-text-slate-700 dark:tw-border-slate-600 dark:tw-bg-slate-800 dark:tw-text-slate-200">
      {children}
    </kbd>
  );
}
...
export function AccentText({children}: {children: ReactNode}) {
  return (
    <span className="tw-text-indigo-600 dark:tw-text-indigo-400">{children}</span>
  );
}
...
export function SectionHead({
  ...
  return (
    <header className="tw-mb-11 tw-max-w-2xl">
      <p className="tw-mb-0 tw-font-mono tw-text-xs tw-font-semibold tw-uppercase tw-tracking-widest tw-text-indigo-600 dark:tw-text-indigo-300">
        {eyebrow}
      </p>
...
export function PrimaryButton({to, children}: {to: string; children: ReactNode}) {
  return (
    <Link
      to={to}
      className="tw-inline-flex tw-items-center tw-gap-2 tw-rounded-lg tw-bg-indigo-600 tw-px-6 tw-py-3 tw-font-semibold tw-text-white tw-no-underline tw-shadow-lg tw-shadow-indigo-600/25 tw-transition hover:tw--translate-y-0.5 hover:tw-bg-indigo-700 hover:tw-text-white hover:tw-no-underline hover:tw-shadow-indigo-600/40 dark:tw-bg-indigo-500 dark:tw-shadow-indigo-500/20 dark:hover:tw-bg-indigo-400">
      {children}
    </Link>
  );
}
